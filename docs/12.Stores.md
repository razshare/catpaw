...
function main() {
    ...
    $unsubscribers[] = $store->subscribe(function($value) use ($counter) {
        //you can execute async code here
        echo "new value received (by subscriber 1): $value".PHP_EOL;
        $counter->set($counter->get() + 1);
    });

    $unsubscribers[] = $store->subscribe(function($value) use ($counter) {
        //you can execute async code here
        echo "new value received (by subscriber 2): $value".PHP_EOL;
        $counter->set($counter->get() + 1);
    });

    $unsubscribers[] = $store->subscribe(function($value) use ($counter) {
        //you can execute async code here
        echo "new value received (by subscriber 3): $value".PHP_EOL;
        $counter->set($counter->get() + 1);
    });
...
```sh
new value received (by subscriber 1): default
new value received (by subscriber 1): hello world
new value received (by subscriber 2): hello world
new value received (by subscriber 3): hello world
