...
1. manipulate `composer.json` to bump up any known dependencies to the latest version.<br/>
  A `known dependencies` means it's a dependency listed in your `product.yml` file, it's name must match the `library` property.<br/>
  For example, the following project's library name is `catpaw/core` and `product.yml` would indicate that as follows:<br/>
    ```yaml
    ../catpaw-core:
      library: catpaw/core
      version: 0.4.30
    ```
    while `composer.json` would require the dependency like so:
    ```json
    "require": {
      "catpaw/core": "^0.4"
    }
...
- `/** @inject */` replaced with prepared statements parameters.<br/>
   These parameters can be injected into the SQL queries using some special comments.
   ```sql
   /** @inject param 'username' into @username */
   select * from account where username like @username
   ```

### `--sql-transform-generator` && `--sql-transform` full example

Executing `php product.phar --sql-transform="./resources/routes/" --sql-transform-generator="./generator.php"` and given the files
```php
// file: generator.php
use Amp\Http\Server\Response;
use Amp\Http\Status;
...
```
```sql
/* file: resources/routes/index.get.sql */
...
```php
// file: resources/routes/index.get.php
