This feature requires the binary of `catpaw-build`, which can be downloaded [here](https://github.com/tncrazvan/catpaw-build/releases).
<hr/>
...
### `--delete-all-tags`
```bash
php build.phar --delete-all-tags
```
...
```php
php build.phar --delete-all-tags --project="./catpaw-web"
```

### `--execute-everywhere`
```bash
php build.phar --execute-everywhere="ls"
```
This will execute `ls` in all projects.

### `--execute-everywhere-parallel`
```bash
php build.phar --execute-everywhere-parallel="ls"
```
This will execute `ls` in all projects in parallel, the output might not be ordered.

### `--start-web-server`
```bash
php build.phar --start-web-server
```
